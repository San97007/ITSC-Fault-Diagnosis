...
archives — an undocumented binary archive does not by itself satisfy a
dataset-transparency request, even though the raw data is present.
...
uncalibrated one -- it cannot be produced by this Python reference
implementation. See `data/table_ablation_dt.csv` and
`manuscript_ablation_study_section.tex` for the placeholder cells and
exact instructions on what to run.
